...
namespace TheCoustRoles.CustomGameModes {
    ...
    class GameModePatches {
        /* 
         Creates a button in the info pane in the lobby to cycle through the game modes of TCR.
         */
        ...
        class LobbyInfoPanePatch {
            ...
            public static void Postfix(LobbyInfoPane __instance) {
                ...
                gameModeButton.name = "TCR GameModeButton";
                ...
                {
                    TCRMapOptions.gameMode = (CustomGamemodes)((int)(TCRMapOptions.gameMode + 1)  % Enum.GetNames(typeof(CustomGamemodes)).Length);
                    __instance.StartCoroutine(Effects.Lerp(0.1f, new Action<float>(p => { pButton.buttonText.text = Helpers.cs(Color.yellow, GameModeText.GetComponent<TextMeshPro>().text); })));
