...
namespace TheCoustRoles.CustomGameModes {
    [HarmonyPatch]
    class PropHunt {
        ...
        public static void clearAndReload() {
            ...
            isPropHuntGM = TCRMapOptions.gameMode == CustomGamemodes.PropHunt;
        ...
        public static Sprite getIntroSprite(int index) {
            return Helpers.loadSpriteFromResources($"TheCoustRoles.Resources.IntroAnimation.intro_{index + 1000}.png", 150f, cache: false);
        }

        public static void updateWhitelistedObjects(bool debug=false) {
            string allNames = Helpers.readTextFromResources("TheCoustRoles.Resources.Txt.Props.txt");
            if (debug) {
        ...
        public static void poolablePlayerUpdate() {
            if (poolablesBackground == null) {
                ...
                if (poolablesBackgroundSprite == null) poolablesBackgroundSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.poolablesBackground.jpg", 200f);
            }
            ...
            foreach (var pc in PlayerControl.AllPlayerControls) {
                if (!TCRMapOptions.playerIcons.ContainsKey(pc.PlayerId)) continue;
                var poolablePlayer = TCRMapOptions.playerIcons[pc.PlayerId];
                if (pc.Data.IsDead) {
        ...
        public static void update() {            
            if (!isPropHuntGM) {
                // Make sure the DangerMeter is not displayed in TCR HideNSeek, Classic or Guesser Game mode.
        ...
        public static Sprite getDisguiseButtonSprite() {
            ...
            disguiseButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.DisguiseButton.png", 115f);
            return disguiseButtonSprite;
        }

        public static Sprite getUnstuckButtonSprite() {
            if (unstuckButtonSprite) return unstuckButtonSprite;
            unstuckButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.UnStuck.png", 115f);
            return unstuckButtonSprite;
        }
        public static Sprite getRevealButtonSprite() {
            if (revealButtonSprite) return revealButtonSprite;
            revealButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Reveal.png", 115f);
            return revealButtonSprite;
        }

        public static Sprite getInvisButtonSprite() {
            if (invisButtonSprite) return invisButtonSprite;
            invisButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.InvisButton.png", 115f);
            return invisButtonSprite;
        }

        public static Sprite getFindButtonSprite() {
            if (findButtonSprite) return findButtonSprite;
            findButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.FindButton.png", 115f);
        ...
        public static Sprite getSpeedboostButtonSprite() {
            ...
            speedboostButtonSprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.SpeedboostButton.png", 115f);
        ...
        public static GameObject FindClosestDisguiseObject(GameObject origin, float radius, bool verbose=false) {
            ...
            try {
                ...
                foreach (Collider2D collider in Physics2D.OverlapCircleAll(origin.transform.position, radius)) {
                    if (verbose) {
                        TheCoustRolesPlugin.Logger.LogMessage($"Nearby Object: {collider.gameObject.name}");
                    }
                    bool whiteListed = false;
                ...
                TheCoustRolesPlugin.Logger.LogError($"Error in find closest disguise object: {e}");
                return null; }
        }
        ...
        public static void IntroCutsceneDestroyPatch(IntroCutscene __instance) {
            ...
            var resourceBundle = assembly.GetManifestResourceStream("TheCoustRoles.Resources.IntroAnimation.intro");
        ...
        public static void MapSetPostfix() {  // Make sure the map in the settings is in sync with the map from li
            if (TCRMapOptions.gameMode != CustomGamemodes.PropHunt && TCRMapOptions.gameMode != CustomGamemodes.HideNSeek || AmongUsClient.Instance.IsGameStarted) return;
