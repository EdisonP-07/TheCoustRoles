    ...
    public static class Helpers
    {
        ...
        public static Texture2D loadTextureFromDisk(string path) {
            try {          
                ...
                TheCoustRolesPlugin.Logger.LogError("Error loading texture from disk: " + path);
            }
            return null;
        }

        /* This function has been removed from TCR because we switched to assetbundles for compressed audio. leaving it here for reference - Gendelo
        public static AudioClip loadAudioClipFromResources(string path, string clipName = "UNNAMED_TCR_AUDIO_CLIP") {
            ...
            // Usage example:
            //AudioClip exampleClip = Helpers.loadAudioClipFromResources("TheCoustRoles.Resources.exampleClip.raw");
        ...
        public static bool shouldShowGhostInfo() {
            return PlayerControl.LocalPlayer != null && PlayerControl.LocalPlayer.Data.IsDead && TCRMapOptions.ghostsSeeInformation || AmongUsClient.Instance.GameState == InnerNet.InnerNetClient.GameStates.Ended;
        }
        ...
        public static bool hidePlayerName(PlayerControl source, PlayerControl target) {
            ...
            else if (!TCRMapOptions.hidePlayerNames) return false; // All names are visible
        ...
        public static MurderAttemptResult checkMuderAttempt(PlayerControl killer, PlayerControl target, bool blockRewind = false, bool ignoreBlank = false, bool ignoreIfKillerIsDead = false, bool ignoreMedic = false) {
            ...
            if (TCRMapOptions.shieldFirstKill && TCRMapOptions.firstKillPlayer == target) return MurderAttemptResult.SuppressKill;
        ...
        public static void shareGameVersion() {
            ...
            writer.Write((byte)TheCoustRolesPlugin.Version.Major);
            writer.Write((byte)TheCoustRolesPlugin.Version.Minor);
            writer.Write((byte)TheCoustRolesPlugin.Version.Build);
            writer.Write(AmongUsClient.Instance.AmHost ? Patches.GameStartManagerPatch.timer : -1f);
            writer.WritePacked(AmongUsClient.Instance.ClientId);
            writer.Write((byte)(TheCoustRolesPlugin.Version.Revision < 0 ? 0xFF : TheCoustRolesPlugin.Version.Revision));
            writer.Write(Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId.ToByteArray());
            AmongUsClient.Instance.FinishRpcImmediately(writer);
            RPCProcedure.versionHandshake(TheCoustRolesPlugin.Version.Major, TheCoustRolesPlugin.Version.Minor, TheCoustRolesPlugin.Version.Build, TheCoustRolesPlugin.Version.Revision, Assembly.GetExecutingAssembly().ManifestModule.ModuleVersionId, AmongUsClient.Instance.ClientId);
        }
        ...
        public static void toggleZoom(bool reset=false) {
            ...
            if (tzGO != null) {
                ...
                rend.sprite = zoomOutStatus ? Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Plus_Button.png", 100f) : Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Minus_Button.png", 100f);
                rendActive.sprite = zoomOutStatus ? Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Plus_ButtonActive.png", 100f) : Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Minus_ButtonActive.png", 100f);
        ...
        public static async Task checkBeta() {
            if (TheCoustRolesPlugin.betaDays > 0) {
                TheCoustRolesPlugin.Logger.LogMessage($"Beta check");
                ...
                TheCoustRolesPlugin.Logger.LogMessage($"Compiled at {compileTime.ToString(CultureInfo.InvariantCulture)}");
                DateTime? now;
                ...
                try {
                    ...
                    if (response.IsSuccessStatusCode)
                        ...
                        TheCoustRolesPlugin.Logger.LogMessage($"Could not get time from server: {response.StatusCode}");
                        now = DateTime.UtcNow; //In case something goes wrong. 
                    }
                } catch (System.Net.Http.HttpRequestException) {
                    now = DateTime.UtcNow;
                }
                if ((now - compileTime)?.TotalDays > TheCoustRolesPlugin.betaDays) {
                    TheCoustRolesPlugin.Logger.LogMessage($"Beta expired!");
                ...
                } else TheCoustRolesPlugin.Logger.LogMessage($"Beta will remain runnable for {TheCoustRolesPlugin.betaDays - (now - compileTime)?.TotalDays} days!");
            }
        }
