...
using static TheCoustRoles.CustomOption;
...
namespace TheCoustRoles {
    public class CustomOption {
        ...
        public CustomOption(int id, CustomOptionType type, string name,  System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, Action onChange = null, string heading = "", bool invertedParent = false) {
            ...
            selection = 0;
            if (id != 0) {
                entry = TheCoustRolesPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
        ...
        public static void switchPreset(int newPreset) {
            ...
            vanillaSettings = TheCoustRolesPlugin.Instance.Config.Bind($"Preset{preset}", "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options) {
                if (option.id == 0) continue;

                option.entry = TheCoustRolesPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
        ...
        public static bool loadVanillaOptions() {
            ...
            if (gameOptions.Version < 8) {
                TheCoustRolesPlugin.Logger.LogMessage("tried to paste old settings, not doing this!");
                return false;
            } 
        ...
        public static int deserializeOptions(byte[] inputValues) {
            ...
            while (reader.BaseStream.Position < inputValues.Length) {
                try {
                    ...
                    TheCoustRolesPlugin.Logger.LogWarning($"id:{lastId}:{e}: while deserializing - tried to paste invalid settings!");
                    errors++;
                }
            }
        ...
        public static void copyToClipboard() {
            GUIUtility.systemCopyBuffer = $"{TheCoustRolesPlugin.VersionString}!{Convert.ToBase64String(serializeOptions())}!{vanillaSettings.Value}";
        }

        public static int pasteFromClipboard() {
            string allSettings = GUIUtility.systemCopyBuffer;
            int tcrOptionsFine = 0;
            bool vanillaOptionsFine = false;
            try {
                var settingsSplit = allSettings.Split("!");
                Version versionInfo = Version.Parse(settingsSplit[0]);
                string tcrSettings = settingsSplit[1];
                string vanillaSettingsSub = settingsSplit[2];
                tcrOptionsFine = deserializeOptions(Convert.FromBase64String(tcrSettings));
                ShareOptionSelections();
                if (TheCoustRolesPlugin.Version > versionInfo && versionInfo < Version.Parse("4.6.0")) {
                    vanillaOptionsFine = false;
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "Host Info: Pasting vanilla settings failed, TCR Options applied!");
                } else {
                    vanillaSettings.Value = vanillaSettingsSub;
                    vanillaOptionsFine = loadVanillaOptions();
                }
            } catch (Exception e) {
                TheCoustRolesPlugin.Logger.LogWarning($"{e}: tried to paste invalid settings!\n{allSettings}");
            ...
            return Convert.ToInt32(vanillaOptionsFine) + tcrOptionsFine;
        }
    }
    ...
    class LobbyViewSettingsPatch{
        ...
        public static void createCustomButton(LobbyViewSettingsPane __instance, int targetMenu, string buttonName, string buttonText, CustomOptionType optionType) {
            ...
            var tcrSettingsButton = GameObject.Find(buttonName);
            if (tcrSettingsButton == null) {
                tcrSettingsButton = GameObject.Instantiate(buttonTemplate, buttonTemplate.transform.parent);
                tcrSettingsButton.transform.localPosition += Vector3.right * 1.75f * (targetMenu - 2);
                tcrSettingsButton.name = buttonName;
                __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p => { tcrSettingsButton.transform.FindChild("FontPlacer").GetComponentInChildren<TextMeshPro>().text = buttonText; })));
                var tcrSettingsPassiveButton = tcrSettingsButton.GetComponent<PassiveButton>();
                tcrSettingsPassiveButton.OnClick.RemoveAllListeners();
                tcrSettingsPassiveButton.OnClick.AddListener((System.Action)(() => {
                    __instance.ChangeTab((StringNames)targetMenu);
                }));
                tcrSettingsPassiveButton.OnMouseOut.RemoveAllListeners();
                tcrSettingsPassiveButton.OnMouseOver.RemoveAllListeners();
                tcrSettingsPassiveButton.SelectButton(false);
                currentButtons.Add(tcrSettingsPassiveButton);
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) {
            ...
            if (TCRMapOptions.gameMode == CustomGamemodes.Guesser) // Exclude guesser options in neutral mode
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance) {
            ...
            int next = 3;
            if (TCRMapOptions.gameMode == CustomGamemodes.Guesser || TCRMapOptions.gameMode == CustomGamemodes.Classic) {

                // create TCR settings
                createCustomButton(__instance, next++, "TCRSettings", "TCR Settings", CustomOptionType.General);
                   // create TCR settings
    ...
    class GameOptionsMenuStartPatch {
        ...
        public static void Postfix(GameSettingMenu __instance) {
            ...
            bgrenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.CopyPasteBG.png", 175f);
            ...
            copyButtonRenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.PasteActive.png", 100f);
        ...
        public static void createCustomButton(GameSettingMenu __instance, int targetMenu, string buttonName, string buttonText) {
            ...
            var tcrSettingsButton = GameObject.Find(buttonName);
            if (tcrSettingsButton == null) {
                tcrSettingsButton = GameObject.Instantiate(buttonTemplate, leftPanel.transform);
                tcrSettingsButton.transform.localPosition += Vector3.up * 0.5f * (targetMenu - 2);
                tcrSettingsButton.name = buttonName;
                __instance.StartCoroutine(Effects.Lerp(2f, new Action<float>(p => { tcrSettingsButton.transform.FindChild("FontPlacer").GetComponentInChildren<TextMeshPro>().text = buttonText; })));
                var tcrSettingsPassiveButton = tcrSettingsButton.GetComponent<PassiveButton>();
                tcrSettingsPassiveButton.OnClick.RemoveAllListeners();
        ...
        public static void createGameOptionsMenu(GameSettingMenu __instance, CustomOptionType optionType, string settingName) {
            ...
            var tcrSettingsTab = GameObject.Instantiate(tabTemplate, tabTemplate.transform.parent);
            tcrSettingsTab.name = settingName;
                
            var tcrSettingsGOM = tcrSettingsTab.GetComponent<GameOptionsMenu>();

            updateGameOptionsMenu(optionType, tcrSettingsGOM);

            currentTabs.Add(tcrSettingsTab);
            tcrSettingsTab.SetActive(false);
            currentGOMs.Add((byte)optionType, tcrSettingsGOM);
        }

        public static void updateGameOptionsMenu(CustomOptionType optionType, GameOptionsMenu tcrSettingsGOM) {
            foreach (var child in tcrSettingsGOM.Children) {
                child.Destroy();
            }
            tcrSettingsGOM.scrollBar.transform.FindChild("SliderInner").DestroyChildren();
            tcrSettingsGOM.Children.Clear();
            var relevantOptions = options.Where(x => x.type == optionType).ToList();
            if (TCRMapOptions.gameMode == CustomGamemodes.Guesser) // Exclude guesser options in neutral mode
                relevantOptions = relevantOptions.Where(x => !(new List<int> { 310, 311, 312, 313, 314, 315, 316, 317, 318 }).Contains(x.id)).ToList();
            createSettings(tcrSettingsGOM, relevantOptions);
        }
        ...
        private static void createSettingTabs(GameSettingMenu __instance) {
            ...
            if (TCRMapOptions.gameMode == CustomGamemodes.Guesser || TCRMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TCR settings
                createCustomButton(__instance, next++, "TCRSettings", "TCR Settings");
                createGameOptionsMenu(__instance, CustomOptionType.General, "TCRSettings");
    ...
    class GameOptionsDataPatch
    {
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            } else if (TCRMapOptions.gameMode == CustomGamemodes.Classic) 
            ...
            foreach (CustomOption option in options) {
                if (TCRMapOptions.gameMode == CustomGamemodes.HideNSeek && option.type != CustomOptionType.HideNSeekMain && option.type != CustomOptionType.HideNSeekRoles) continue;
                if (TCRMapOptions.gameMode == CustomGamemodes.PropHunt && option.type != CustomOptionType.PropHunt) continue;
                if (option.parent != null) {
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            int counter = TheCoustRolesPlugin.optionsPage;
            ...
            if (TCRMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                if (TheCoustRolesPlugin.optionsPage > 1) TheCoustRolesPlugin.optionsPage = 0;
                maxPage = 2;
                switch (counter) {
                    case 0:
    ...
    public static class GameOptionsNextPagePatch
    {
        ...
        {
            int page = TheCoustRolesPlugin.optionsPage;
            if (Input.GetKeyDown(KeyCode.Tab)) {
                TheCoustRolesPlugin.optionsPage = (TheCoustRolesPlugin.optionsPage + 1) % 7;
            }
            if (Input.GetKeyDown(KeyCode.Alpha1) || Input.GetKeyDown(KeyCode.Keypad1)) {
                TheCoustRolesPlugin.optionsPage = 0;
            }
            if (Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Keypad2)) {
                TheCoustRolesPlugin.optionsPage = 1;
            }
            if (Input.GetKeyDown(KeyCode.Alpha3) || Input.GetKeyDown(KeyCode.Keypad3)) {
                TheCoustRolesPlugin.optionsPage = 2;
            }
            if (Input.GetKeyDown(KeyCode.Alpha4) || Input.GetKeyDown(KeyCode.Keypad4)) {
                TheCoustRolesPlugin.optionsPage = 3;
            }
            if (Input.GetKeyDown(KeyCode.Alpha5) || Input.GetKeyDown(KeyCode.Keypad5)) {
                TheCoustRolesPlugin.optionsPage = 4;
            }
            if (Input.GetKeyDown(KeyCode.Alpha6) || Input.GetKeyDown(KeyCode.Keypad6)) {
                TheCoustRolesPlugin.optionsPage = 5;
            }
            if (Input.GetKeyDown(KeyCode.Alpha7) || Input.GetKeyDown(KeyCode.Keypad7)) {
                TheCoustRolesPlugin.optionsPage = 6;
            }
            ...
            if (TheCoustRolesPlugin.optionsPage >= GameOptionsDataPatch.maxPage) TheCoustRolesPlugin.optionsPage = 0;
        }
    }
    ...
    public class HudManagerUpdate {
        ...
        public static void Postfix(HudManager __instance) {
            ...
            if (!toggleSettingsButton || !toggleSettingsButtonObject) {
                ...
                renderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Settings_ButtonActive.png", 100);
            ...
            if (!toggleZoomButton || !toggleZoomButtonObject) {
                ...
                tZrenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Minus_ButtonActive.png", 100);
        ...
        public static void Postfix2(HudManager __instance) {
            ...
            if (!toggleSummaryButton || !toggleSummaryButtonObject) {
                ...
                renderer.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.Endscreen.png", 100f);
                rendererActive.sprite = Helpers.loadSpriteFromResources("TheCoustRoles.Resources.EndscreenActive.png", 100f);
