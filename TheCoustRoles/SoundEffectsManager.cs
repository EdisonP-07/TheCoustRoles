...
namespace TheCoustRoles
{
        ...
        
    {
        ...
        public static void Load()
        {
            ...
            {
                if (resourceName.Contains("TheCoustRoles.Resources.SoundEffects.") && (resourceName.Contains(".raw") || resourceName.Contains(".ogg")))
                {
                    soundEffects.Add(resourceName, Helpers.loadAudioClipFromResources(resourceName));
                }
            }*/

            var resourceBundle = assembly.GetManifestResourceStream("TheCoustRoles.Resources.SoundEffects.toraudio");
        ...
        public static AudioClip get(string path)
        {
            ...
            //if (!path.Contains(".")) path = "TheCoustRoles.Resources.SoundEffects." + path + ".raw";
        ...
        {
            if (!TCRMapOptions.enableSoundEffects) return null;
        ...
        public static void playAtPosition(string path, Vector2 position, float maxDuration = 15f, float range = 5f, bool loop = false) {
            if (!TCRMapOptions.enableSoundEffects || !Constants.ShouldPlaySfx()) return;
            AudioClip clipToPlay = get(path);
            TheCoustRolesPlugin.Logger.LogMessage("play at  position");
            if (clipToPlay == null) {
                TheCoustRolesPlugin.Logger.LogMessage("clip is null");
                return;
            }

            AudioSource source = SoundManager.Instance.PlaySound(clipToPlay, false, 1f);
            if (source == null) {
                TheCoustRolesPlugin.Logger.LogMessage("source is null");
                return;
            }
            //currentSources.Add(source);
            source.loop = loop;
            ...
            TheCoustRolesPlugin.Logger.LogMessage("end play at position");
        }
        ...
        public static void stop(string path) {
            ...
            if (soundToStop != null) {
                ...
                catch (Exception e) { TheCoustRolesPlugin.Logger.LogWarning($"Exception in stop sound: {e}"); }
            } 
        }
